...
A Monte Carlo that crosses four estimators of the autoregressive coefficient
with two data-generating processes, and tracks bias, dispersion and coverage as
the panel length `T` varies. Three of the four are inconsistent somewhere; the
fourth, GMM on the full covariance matrix of growth, is the reference that shows
what the others are missing, and it recovers σ_ε along the way.
...
| Path | Contents |
| --- | --- |
| `output/pilot/<model>_T<T>_N<N>.parquet` | raw ρ̂ and clustered SE for every replication (and σ̂_ε where the estimator reports one), stamped with the spec hash and git commit |
| `output/summary.csv` | mean bias and its Monte Carlo SE, SD, RMSE, coverage, mean SE / SD, per cell — for ρ̂ and for σ̂_ε |
...
| `output/table_sigma_eps.md` | mean σ̂_ε (SD) [plim] and its coverage; GMM rows only |
| `output/figures/fig1_mean_rho_by_T.*` | Figure 1: mean ρ̂ against T, two panels, dashed plims |
| `output/figures/fig2_density_T10.*` | Figure 2: densities of ρ̂ at T = 10 for the eight cells |
...
src/sim/
  ...
  analytics.py    the plims, and Omega: the covariance matrix of growth
  dgp.py          seeds and the N x (T+1) panel
  estimators.py   three closed-form ratios of sums, plus the GMM that fits Omega
